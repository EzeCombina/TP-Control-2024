...
#define FILTRO              1000
#define ADC_CHANNEL         ADC1_CHANNEL_4      // Corresponde al GPIO 32
#define ADC_CHANNEL_REF     ADC1_CHANNEL_5      // Corresponde al GPIO 34
#define ADC_ATTEN           ADC_ATTEN_DB_12     // Atenuación de 12 dB en el voltaje de entrada para usar el ADC
#define MUESTRAS            64                  // Muestras para obtener un promedio del valor del ADC
#define T_MOTOR             21.5
#define BITS12              4096
...
int LecturaCruda_REF            = 0;
int LecturaFiltrada_REF         = 0;
double Filtro_REF               = 0;
...
void app_main() {
    ...
    // Configuración del canal y atenuación del ADC
    ESP_ERROR_CHECK(adc1_config_channel_atten(ADC_CHANNEL, ADC_ATTEN));
    ESP_ERROR_CHECK(adc1_config_channel_atten(ADC_CHANNEL_REF, ADC_ATTEN));
    ...
    while (1) {
        ...
        if(modo == ESCALON)
        {
            ...
            LecturaCruda_REF = adc1_get_raw(ADC_CHANNEL_REF);
            Filtro_REF += ((double)LecturaCruda_REF - Filtro_REF) / (double)FILTRO;
            LecturaFiltrada_REF = (int)Filtro_REF;

            REF = (LecturaFiltrada_REF*255.0)/4096.0;
        }
